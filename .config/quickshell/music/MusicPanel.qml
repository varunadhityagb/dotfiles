...
Item {
    ...
    property bool lyricVisible: false
    ...
    Process {
        id: lyricPoller
        command: ["bash", "-c", "cat /tmp/qs_current_lyric 2>/dev/null"]
        stdout: StdioCollector {
            onStreamFinished: {
                let line = this.text.trim()
                root.currentLyric = line
                root.lyricVisible = line.length > 0 && root.musicData.status === "Playing"
            }
        }
    }
    ...
    Timer {
        ...
        triggeredOnStart: true
        onTriggered: {
            if (!lyricPoller.running) lyricPoller.running = true;
        }
    }
    ...
    Item {
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                Item {
                    Layout.fillWidth: true
                    Layout.preferredHeight: root.lyricVisible ? lyricInner.implicitHeight + 24 : 0
                    Layout.bottomMargin: 20
                    clip: true

                    Behavior on Layout.preferredHeight { NumberAnimation { duration: 400; easing.type: Easing.OutQuart } }

                    opacity: root.lyricVisible ? 1.0 : 0.0
                ...
                Rectangle {
                    Layout.fillWidth: true
                    Layout.preferredHeight: 2
                    Layout.topMargin: root.lyricVisible ? 0 : 20
                    Layout.bottomMargin: 20
                    color: "#1AFFFFFF"
                    radius: 1
                    Behavior on Layout.topMargin { NumberAnimation { duration: 400; easing.type: Easing.OutQuart } }
